...
class ProcrustesAligner {
   public:
    Matrix4f estimatePose(const std::vector<Vector3f>& sourcePoints,
        ...
        //    y = R (x' - yMean) + yMean = R (x + t - yMean) + yMean = R x + (R
        //    t - R yMean + yMean)

        Matrix4f estimatedPose = Matrix4f::Identity();
        estimatedPose.block(0, 0, 3, 3) = rotation;
        estimatedPose.block(0, 3, 3, 1) =
            rotation * translation - rotation * targetMean + targetMean;
   ...
   private:
    ...
    Matrix3f estimateRotation(const std::vector<Vector3f>& sourcePoints,
        ...
        // Estimate the rotation from source to target points, following the
        // Procrustes algorithm. To compute the singular value decomposition you
        // can use JacobiSVD() from Eigen.
        const unsigned nPoints = sourcePoints.size();
        MatrixXf sourceMatrix(nPoints, 3);
        MatrixXf targetMatrix(nPoints, 3);

        for (int i = 0; i < nPoints; ++i) {
            sourceMatrix.block(i, 0, 1, 3) =
                (sourcePoints[i] - sourceMean).transpose();
            targetMatrix.block(i, 0, 1, 3) =
                (targetPoints[i] - targetMean).transpose();
        }
        ...
        Matrix3f R =
            U * D * V.transpose();  // the multiplication by D is necessary
                                    // since UV' is only orthogonal, but not
