...
PointCloud ray_marching(const TSDFVolume &tsdf, VirtualSensor &sensor, const Matrix4f& current_pose_estimate) {
    ...
    Vector3f hurka = current_pose_estimate.block(0, 0, 3, 3) * sensor.getDepthIntrinsics().inverse() * Vector3f(320, 240, 1.0f);
...
//            Vector3f origin = current_pose_estimate.block(0, 3, 3, 1);
            ...
            Vector3f ray_direction = current_pose_estimate.block(0, 0, 3, 3) * sensor.getDepthIntrinsics().inverse() * Vector3f(x, y, 1.0f);
    ...
    float maximum = -*std::min_element(distances_copy.begin(), distances_copy.end());
    float minimum = -*std::max_element(distances_copy.begin(), distances_copy.end());
