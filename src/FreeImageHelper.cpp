...
bool FreeImage::LoadImageFromFile(const std::string& filename,
    ...
    if ((bits == 0) || (w == 0) || (h == 0)) return false;
    ...
    for (unsigned int j = 0; j < h; j++) {
        for (unsigned int i = 0; i < w; i++) {
            ...
            for (unsigned int c = 0; c < nChannels && c < 3; ++c) {
                // col[c] = std::min(std::max(0, (int)(255.0f*data[nChannels *
                // (w*j + i) + c])), 255);
                col[c] = std::min(
                    std::max(0,
                             (int)(255.0f * data[nChannels * (w * j + i) + c])),
                    255);
            }
...
bool FreeImageB::LoadImageFromFile(const std::string& filename,
    ...
    // if this somehow one of these failed (they shouldn't), return failure
    if ((bitsF == 0) || (w == 0) || (h == 0)) return false;
