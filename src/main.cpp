...
using VertexAction = std::function<void(const Vertex &, const Vertex &, const Vertex &, const Vertex &)>;
...
void
processVertex(unsigned int idx, unsigned int width, Vertex *vertices, float edgeThreshold, const VertexAction &action) {
...
bool WriteMesh(Vertex *vertices, unsigned int width, unsigned int height, const std::string &filename) {
    ...
    unsigned nFaces = 0;
    VertexAction countFaces = [&nFaces](const Vertex &, const Vertex &, const Vertex &, const Vertex &) {
        nFaces += 2;
    };
    ...
    for (unsigned int y = 0; y < height; ++y) {
        for (unsigned int x = 0; x < width; ++x) {
            ...
            if (v.position.x() != MINF) {
                outFile << v.position.x() << " " << v.position.y() << " " << v.position.z() << " " << (int) v.color.x()
                        << " " << (int) v.color.y() << " " << (int) v.color.z() << " " << (int) v.color.w()
                        << std::endl;
            } else {
    ...
    VertexAction writeFaces = [&outFile, &vertices](const Vertex &v0, const Vertex &v1, const Vertex &v2,
                                                    const Vertex &v3) {
        ...
        for (unsigned int y = 0; y < sensor.GetDepthImageHeight(); ++y) {
            for (unsigned int x = 0; x < sensor.GetDepthImageWidth(); ++x) {
                ...
                if (depth == MINF) {
                    ...
                    Vector4f p = Vector4f(depth * (x - cX) / fX, depth * (y - cY) / fY, depth, 1.0f);
                    Vector4f p_world = trajectoryInv * depthExtrinsicsInv * p;
                    vertices[idx].position = p_world;
                    vertices[idx].color = Vector4uc(colorMap[idx * 4], colorMap[idx * 4 + 1], colorMap[idx * 4 + 2],
                                                    colorMap[idx * 4 + 3]);
                }
            }
        }
