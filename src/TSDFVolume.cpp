...
void TSDFVolume::integrate(const PointCloud& pointCloud, float truncationDistance) {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        if (voxelCoord[0] >= 0 && voxelCoord[0] < width &&
            ...
            float sdf = normal.dot(point - voxelCoord.cast<float>() * voxelSize);
            sdf = std::min(std::max(sdf, -truncationDistance), truncationDistance);

            // Weighted average update
            float wNew = 1.0; // Example: constant weight
            voxel.distance = (voxel.distance * voxel.weight + sdf * wNew) / (voxel.weight + wNew);
            voxel.weight += wNew;
        }
    }
}
