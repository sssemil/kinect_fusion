...
Triangle::Triangle() : idx0(0), idx1(0), idx2(0) {}

Triangle::Triangle(unsigned int _idx0, unsigned int _idx1, unsigned int _idx2)
...
unsigned int SimpleMesh::addFace(unsigned int idx0, unsigned int idx1,
                                 unsigned int idx2) {
    auto fId = static_cast<unsigned int>(m_triangles.size());
    Triangle triangle(idx0, idx1, idx2);
    m_triangles.push_back(triangle);
    return fId;
}

std::vector<Vertex>& SimpleMesh::getVertices() { return m_vertices; }

const std::vector<Vertex>& SimpleMesh::getVertices() const {
    return m_vertices;
}

std::vector<Triangle>& SimpleMesh::getTriangles() { return m_triangles; }

const std::vector<Triangle>& SimpleMesh::getTriangles() const {
    return m_triangles;
}

void SimpleMesh::transform(const Matrix4f& transformation) {
    for (Vertex& v : m_vertices) {
        v.position = transformation * v.position;
    }
}

bool SimpleMesh::loadMesh(const std::string& filename) {
    ...
    } else if (std::string(string1) == "OFF") {
...
bool SimpleMesh::writeMesh(const std::string& filename) {
    ...
    for (const auto& vertex : m_vertices) {
        if (vertex.position.allFinite())
            outFile << vertex.position.x() << " " << vertex.position.y() << " "
                    ...
                    << " " << int(vertex.color.y()) << " "
                    << int(vertex.color.z()) << " " << int(vertex.color.w())
                    << std::endl;
        else
    ...
    for (auto& m_triangle : m_triangles) {
        outFile << "3 " << m_triangle.idx0 << " " << m_triangle.idx1 << " "
                << m_triangle.idx2 << std::endl;
    }
 ...
 */
SimpleMesh SimpleMesh::joinMeshes(const SimpleMesh& mesh1,
                                  const SimpleMesh& mesh2,
                                  const Matrix4f& pose1to2) {
    ...
    const auto nVertices1 = static_cast<unsigned>(vertices1.size());
    const auto nVertices2 = static_cast<unsigned>(vertices2.size());
    joinedVertices.reserve(nVertices1 + nVertices2);

    const auto nTriangles1 = static_cast<unsigned>(triangles1.size());
    const auto nTriangles2 = static_cast<unsigned>(triangles2.size());
 ...
 */
SimpleMesh SimpleMesh::sphere(Vector3f center, float scale,
                              const Vector4uc& color) {
    SimpleMesh mesh;
 ...
 */
SimpleMesh SimpleMesh::camera(const Matrix4f& cameraPose, float scale,
                              const Vector4uc& color) {
    SimpleMesh mesh;
 ...
 */
SimpleMesh SimpleMesh::cylinder(const Vector3f& p0, const Vector3f& p1,
    ...
    for (unsigned i = 0; i <= stacks; i++)
        for (unsigned i2 = 0; i2 < slices; i2++) {
            ...
            v.position = Vector4f{
                p0.x() + radius * cosf(theta), p0.y() + radius * sinf(theta),
 ...
 */
Matrix3f SimpleMesh::face(const Vector3f& vA, const Vector3f& vB) {
    ...
    Matrix3f rotation =
        ...
        (1 - std::cos(angle)) * skewSymetricMatrix * skewSymetricMatrix;
